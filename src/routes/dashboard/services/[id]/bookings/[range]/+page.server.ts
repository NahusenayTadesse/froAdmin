...
import { and, asc, eq, sql, between, getTableColumns, aliasedTable } from 'drizzle-orm';
...
export const load: PageServerLoad = async ({ params }) => {
	...
	const allTransactions = await db
		...
		.where(
			and(eq(bookings.serviceId, id), between(bookings.createdAt, new Date(start), new Date(end)))
		)
