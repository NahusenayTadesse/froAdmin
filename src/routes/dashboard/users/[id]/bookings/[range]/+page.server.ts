...
export const load: PageServerLoad = async ({ params }) => {
	...
	const allTransactions = await db
		.select({
			...
			providerName: sql<string>`concat(${profiles.firstName}, ' ', ${profiles.lastName})`,
			serviceName: services.title
		})
		.from(bookings)
		.leftJoin(services, eq(services.id, bookings.serviceId))
		.leftJoin(profiles, eq(profiles.id, bookings.providerId))
		.where(
			and(eq(bookings.customerId, id), between(bookings.createdAt, new Date(start), new Date(end)))
		)
