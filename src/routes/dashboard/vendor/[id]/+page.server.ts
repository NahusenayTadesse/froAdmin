...
import { banUserSchema as ban, unBanUserSchema, verifyUserSchema } from '$lib/ZodSchema';
...
export const load: PageServerLoad = async ({ params }) => {
	...
	const verifyForm = await superValidate(zod4(verifyUserSchema));
...
export const actions: Actions = {
	...
	verify: async ({ request, params }) => {
		const form = await superValidate(request, zod4(verifyUserSchema));
		...
		const { verify, verificationStatus } = form.data;

		try {
			await db
				.update(user)
				.set({
					isVerifiedProvider: verify,
					verificationStatus
				})
				.where(eq(user.id, id));
