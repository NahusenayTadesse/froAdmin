...
export const load: PageServerLoad = async ({ url }) => {
	try {
		...
		const verificationStatus = getStringParam(url, 'verificationStatus');
		const isVerifiedProvider = getBooleanParam(url, 'isVerifiedProvider');
		...
		const whereConditions = and(
			...
			verificationStatus ? eq(user.verificationStatus, verificationStatus) : undefined,

			isVerifiedProvider !== null ? eq(user.isVerifiedProvider, isVerifiedProvider) : undefined,
