<script>
	...
	// lightweight local pluralize fallback to avoid requiring the 'pluralize' package
	function pluralize(word) {
		if (!word) return '';
		// very small heuristic: don't add 's' if it already ends with s, otherwise append 's'
		// keep casing intact so the template can run the same replace(/([a-z])([A-Z])/g, '$1 $2')
